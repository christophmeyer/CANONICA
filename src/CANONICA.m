...
AlgebraicallyIndepQ[polyList_List, invariants_List, OptionsPattern[]] :=
	Module[	{annihilatingPoly},
		annihilatingPoly =
			GroebnerBasis[MapIndexed[Y[#2[[1]]] - # &, polyList],
			Join[invariants, Table[Y[i], {i, 1, Length[polyList]}]], invariants];
...
ApplyRelations[termList_List, alphabet_List, invariants_List] :=
	Module[ {},
		...
		Return[
			Times @@ (#[[2]] /. Fac[nLetter_, pow_] :> Power[alphabet[[nLetter]], pow]) & /@
			FindAndApplyRelations[alphabet, invariants,
			Map[{RandomReal[{0, 1}, WorkingPrecision -> MachinePrecision], #[[2]]} &,
			Flatten[ToIndepFactorForm[#, alphabet, invariants] & /@ termList, 1]], {}]
		];
	];
...
preferredTerms_, OptionsPattern[]] :=
	Module[ {indepFactorForm, rawForm},
		...
		If[ OptionValue[Form] === IndepFactor,
			...
			rawForm =
				Map[{#[[1]], Times @@ #[[2]]} &, indepFactorForm /. Fac[Letter_, Pow_] :> Power[alphabet[[Letter]], Pow]];
			Return[rawForm];
		];
	];
...
OptionsPattern[]] :=
	Module[ {alphabet, bhat, negPowerFactors,
		...
		If[ !(IntegerQ[OptionValue[DDeltaDenominatorDegree]] && OptionValue[DDeltaDenominatorDegree] >= 0),
			Message[CalcAnsatzD::nintden];
			Return[False];
		];

		If[ !(IntegerQ[OptionValue[DDeltaNumeratorDegree]] && OptionValue[DDeltaNumeratorDegree] >= 0),
		...
		dfacInitLetters =
			Table[Map[Function[list, Select[list, #[[1]] === nLetter &]], dfacInit, {2}], {nLetter, 1, Length@alphabet}];

		negPowerFactors =
			Union[Flatten[
					Function[expr, CalcNegPowerPossibilities[alphabet, invariants, expr]] /@
					Flatten[MapThread[Union, FixedPoint[
		...
		posPowerFactors =
			Join[{1},
				Union[Apply[Times, Flatten[
				Table[Tuples[DeleteCases[Variables[Join[bhat, c, e]], eps],
					{nLetters}], {nLetters, 1, 3 + OptionValue[DDeltaNumeratorDegree]}
				], 1], {1}]
				]
			];
...
CalcbhatAndFactors[b_List, (*alphabet*)_List, invariants_List] :=
	Module[ {occuringSpuriousFactors, hbar, epsDepFactors,
		...
		occuringOverallFactors =
			...
			SameTest -> (NumberQ[Simplify[#1[[1]]/#2[[1]]]] && #1[[2]] == #2[[2]] &)], #[[1]] &];
	...
	Module[ {adjMatrix, list, revTrafo, preList, sectorBoundaries},
		...
		sectorBoundaries =
			...
			Expr, {Append[
				...
				1]}], {{},
			Map[{#[[1]], #[[1]] === #[[2]]} &, preList]}, #[[2]] =!= {} &][[1]];

		Return[CalcInverseBlockTriangular[revTrafo.trafo, sectorBoundaries].revTrafo];
	];
...
CalcInverseBlockTriangular[trafo_List, sectorBoundaries_List] :=
	Module[	{prevSecInverse, prevSecTrafo},
		...
		prevSecTrafo =
			Simplify[InsertBlockIntoIdentity[prevSecInverse, Length@trafo, 1].trafo];

		Return[Nest[CalcInverseStep, {sectorBoundaries, prevSecInverse,
			prevSecTrafo}, Length@sectorBoundaries - 1][[2]]];
	];


CalcInverseStep[{sectorBoundaries_List, prevSecInverse_List, prevSecTrafo_List}] :=
	Module[	{prevSubSector, subSector, nextOffDiagonalBlock, nextDiagonalBlock,
			nextDiagonalBlockInv, nextInvStep, nextCurrentInv, nextTrafo},
		...
		nextOffDiagonalBlock =
			Transpose[Take[Transpose[Take[prevSecTrafo, sectorBoundaries[[subSector]]]],
				{1, sectorBoundaries[[subSector]][[1]] - 1}]];
		...
		nextInvStep =
			Join[
				Drop[IdentityMatrix[sectorBoundaries[[subSector]][[2]]], -Length@nextDiagonalBlock],
				MapThread[Join, {-nextDiagonalBlockInv.nextOffDiagonalBlock,nextDiagonalBlockInv}]
			];

		nextCurrentInv =
			nextInvStep.InsertBlockIntoIdentity[prevSecInverse, sectorBoundaries[[subSector]][[2]], 1];

		nextTrafo =
			InsertBlockIntoIdentity[nextInvStep, sectorBoundaries[[-1]][[2]], 1].prevSecTrafo;
...
CalcNextDn[e_List, c_List, bhat_List, hbar_, khat_, nmin_Integer, alphabet_List, invariants_List,
	...
	Module[ {nOrder, ansatzD, lMax, kMax, khatMax, equationSet, vars, linearSystem, preSol, solRule,
		...
		ansatzD =
			Table[Table[Total[Table[\[Beta][nOrder, nIndepFactor, i, j]*indepFactors[[nIndepFactor]], {nIndepFactor, 1, Length[indepFactors]}]],
				{j, 1, Length[bhat[[1, 1]]]}], {i, 1, Length[bhat[[1]]]}];
		...
		equationSet =
			Flatten[Table[
			Sum[(-D[SeriesCoefficient[hbar, {eps, 0, k}], invariants[[nVar]]]*Append[previousDs, ansatzD][[nOrder - k - nmin + 2]] +
				SeriesCoefficient[hbar, {eps, 0, k}]*D[Append[previousDs, ansatzD][[nOrder - k - nmin + 2]], invariants[[nVar]]]), {k, 0,
				Min[lMax, nOrder - nmin + 1]}] - Sum[SeriesCoefficient[hbar, {eps, 0, k}]*(e[[nVar]].Append[previousDs, ansatzD][[nOrder - 1 - k - nmin + 2]] -
				Append[previousDs, ansatzD][[nOrder - 1 - k - nmin + 2]].c[[nVar]]), {k, 0, Min[lMax, nOrder - nmin]}] +
				-Sum[g[k]*SeriesCoefficient[bhat[[nVar]], {eps, 0, nOrder - k}], {k, 0,nOrder - nmin}] -
				Sum[D[Total[Table[Log[alphabet[[nLetter]]] Table[Table[\[Alpha][nLetter, nOrder - k, i, j], {j, 1, Length[bhat[[1, 1]]]}], {i, 1, Length[bhat[[1]]]}],
					{nLetter, 1, Length[alphabet]}]], invariants[[nVar]]]*SeriesCoefficient[Power[hbar, 2], {eps, 0, k}], {k, 0, Min[2*lMax, nOrder - nmin]}],
				{nVar, 1, Length[invariants]}]] /. Dispatch@previousSolution;

		vars =
			Join[
				Select[Variables[equationSet], Head[#1] === \[Alpha] &],
				Select[Variables[equationSet], Head[#1] === \[Beta] &],
				Select[Variables[equationSet], Head[#1] === g &]
			];
		linearSystem = (#1 == 0 &) /@
			Flatten[ParallelizeMap[$ComputeParallel][RatFunctionZeroCoeffs[#1, invariants] &, equationSet,
				DistributedContextsSwitch[$ComputeParallel]]];

		If[ OptionValue[VerbosityLevel] >= 12,
			Print["Found " <> ToString[Length[linearSystem]] <> " equations."];
		];
		...
		newPreviousDs =
			Join[previousDs /. Dispatch@newPreviousSolution, {ansatzD /. Dispatch@newPreviousSolution}];
		...
		nextDsVanishResult =
			CheckNextDsVanish[newPreviousDs, newPreviousSolution, bhat, lMax, nmin,
			hbar, e, c, alphabet, invariants, lMax + 1 + Max[kMax + 1, 2*lMax + 1]];
		...
		maxEqnOrder =
			Length[newPreviousDs] + nmin - 2 + lMax + 1 + Max[kMax + 1, 2*lMax + 1];

		gVanishQ = (0 == #1[[2]] &) /@
			Select[nextDsVanishResult, Head[#1[[1]]] === g &&
				MemberQ[Table[i, {i, maxEqnOrder - nmin - (kMax + 1) + 1, maxEqnOrder - nmin}], #1[[1, 1]]] &] /.
			Table[g[i] -> 0, {i, maxEqnOrder - nmin - (kMax + 1) + 1, maxEqnOrder - nmin}] /.
			Table[\[Alpha][_, i, _, _] -> 0, {i, maxEqnOrder - (2 lMax + 1) + 1, maxEqnOrder}];

		bprimehatVanishQ = (0 == #1[[2]] &) /@
			Select[nextDsVanishResult, Head[#1[[1]]] === \[Alpha] &&
				MemberQ[Table[i, {i, maxEqnOrder - (2 lMax + 1) + 1, maxEqnOrder}], #1[[1, 2]]] &
			] /.
			Table[\[Alpha][_, i, _, _] -> 0, {i, maxEqnOrder - (2 lMax + 1) + 1, maxEqnOrder}] /.
			Table[g[i] -> 0, {i, maxEqnOrder - nmin - (kMax + 1) + 1, maxEqnOrder - nmin}];
		...
		If[ Length[stopSol] > 0,
			...
			preCompleteSoln = Join[nextDsVanishResult /. stopSol[[1]], stopSol[[1]]];
			freeVarsToZero = # -> 0 & /@ Variables[#[[2]] & /@ preCompleteSoln];
			freeVarsZeroSol = Join[stopSol[[1]] /. freeVarsToZero, freeVarsToZero];
			completeSoln = Join[nextDsVanishResult /. freeVarsZeroSol, freeVarsZeroSol];
			Return[{newPreviousDs /. Dispatch@completeSoln /. {\[Beta][___] -> 0}, {},
				{Sum[g[i]*Power[eps, i], {i, 0, maxEqnOrder - nmin}] /.
...
CalcNextTn[aHat_List, f_, alphabet_List, invariants_List, lMIN_Integer, lMAX_Integer,
	...
	Module[ {nOrder, tAnsatz, tVector, deqAtNOrder, equationSet, parameterEquations,
		...
		tAnsatz =
			Table[Table[Total[Table[\[Beta][nOrder, nIndepFactor, i, j]*indepFactors[[nIndepFactor]], {nIndepFactor, 1, Length[indepFactors]}]],
				{j, 1, Length[aHat[[1]]]}], {i, 1, Length[aHat[[1]]]}];

		tVector = Append[previousTs, tAnsatz];
		deqAtNOrder =
			Table[
				Sum[
					(-D[SeriesCoefficient[f, {eps, 0, k}], invariants[[nVar]]]*tVector[[nOrder - k - lMIN + 1]] +
					SeriesCoefficient[f, {eps, 0, k}]*D[tVector[[nOrder - k - lMIN + 1]], invariants[[nVar]]]), {k, lMIN,Min[lMAX, nOrder - lMIN]}] -
					Sum[SeriesCoefficient[aHat[[nVar]], {eps, 0, k}].tVector[[nOrder - k - lMIN + 1]], {k, 0, Min[kMAX, nOrder - lMIN]}] +
					Sum[
						(SeriesCoefficient[f, {eps, 0, k}]*tVector[[nOrder - k - 1 - lMIN + 1]].D[Total[Table[
							Log[alphabet[[nLetter]]] Table[Table[\[Alpha][nLetter, i, j], {j, 1,Length[aHat[[1]]]}], {i, 1, Length[aHat[[1]]]}],
							{nLetter, 1,Length[alphabet]}]], invariants[[nVar]]] /.
						Dispatch@previousSolution), {k, lMIN, Min[lMAX, nOrder - 1 - lMIN]}
					], {nVar, 1, Length[invariants]}
			];
		...
		If[ OptionValue[VerbosityLevel] >= 12,
			Print["Found " <> ToString[Length[parameterEquations]] <> " equations."]
		];
		...
		If[ linearPartSolved === {},
			If[ OptionValue[VerbosityLevel] >= 2,
				Print["No solution found at order eps^" <> ToString[nOrder] <> ". Try different Ansatz."]
			];
			Return[False];
		];
		...
		solveResult =
			...
			Union[Flatten[
				Table[Table[Table[\[Beta][lMIN, NIndepFactor, i, j], {NIndepFactor, 1, Length[indepFactors]}],
					{i, 1, Length[aHat[[1]]]}], {j, 1, Length[aHat[[1]]]}]] /.
		...
		newTs =
			Join[previousTs /. Dispatch[solveResult[[1]]], {tAnsatz /. Dispatch[solveResult[[1]]]}];
		newSols =
			Join[previousSolution /. Dispatch[solveResult[[1]]], solveResult[[1]]];
...
CalcOccuringDenLetters[alphabet_List, invariants_List, expr_, delta_Integer] :=
	Module[ {},
		Return[Select[({#[[1]], #[[2]] + 1 - delta} &) /@ ({
		ExprPropLetterQ[#[[1]], invariants, alphabet, ShowLetter -> True][[3]], -#1[[2]]} &) /@
		Select[FactorList[Denominator[expr]], !IndependentOfInvariantsQ[#[[1]],
				invariants] &], #[[2]] <= -1 &]];
	];

CalculateDlogForm[a_List, invariants_List, alphabet_List] :=
	Module[ {irredList, zeroRules, genericdLogForm, eqnsList, solList, nonzeroRules},
		...
		irredList =
			...
			Function[res,
				If[ res === {},
					...
					res
				]][ExtractIrreducibles[expr, AllowEpsDependence -> True]]], Transpose[a, {3, 1, 2}], {2}];

		If[ !FreeQ[irredList, False],
			Return[False];
		];

		If[ ! And @@ (ExprPropLetterQ[#1, invariants, alphabet] &) /@ Union[Flatten[irredList]],
		...
		If[ IndependentOfInvariantsQ[irredList, invariants],
			If[ Union[Flatten[a]] === {0},
				Return[Table[Table[Table[0, {j, 1, Length[a[[1, 1]]]}], {i, 1, Length[a[[1]]]}],
					{nLetter, 1, Length@alphabet}]];,
				Return[False];
			];
		];
		...
		zeroRules =
			...
			Map[ExprPropLetterQ[#1, invariants, alphabet, ShowLetter -> True][[3]] &, irredList, {3}], {2}], {3}]];

		genericdLogForm =
			D[Total[MapIndexed[Table[Table[\[Alpha][#2[[1]], i, j], {j, 1, Length[a[[1, 1]]]}], {i, 1, Length[a[[1]]]}] Log[#1] &,
				alphabet]], #1] & /@ invariants /. Dispatch[zeroRules];

		eqnsList = (Union[Flatten[#1]] &) /@ Map[RatFunctionZeroCoeffs[#1, invariants] &,
			DeleteCases[Flatten[Transpose[genericdLogForm - a, {3, 1, 2}], 1], Table[0, {i, 1, Length[invariants]}]], {2}];

		solList =
			Function[eqns, Solve[Function[lhs, lhs == 0] /@ eqns, DeleteCases[Variables[eqns], eps]]] /@ eqnsList;
		...
		Return[
			MapIndexed[Table[Table[\[Alpha][#2[[1]], i, j], {j, 1, Length[a[[1, 1]]]}],
				{i, 1, Length[a[[1]]]}] &,alphabet] /. Dispatch[zeroRules] /. Dispatch[nonzeroRules]
		];
	];


CalculateNexta[aFull_List, invariants_List, sectorBoundaries_List, trafoPrevious_List, aPrevious_List] :=
	Module[ {sizeNew, sizeOld, rawBlock, subBlock, diagBlock, aNew},
		...
		If[ {{}, {{}}} =!= {trafoPrevious, aPrevious},
			...
			If[ Position[sectorBoundaries, Length[trafoPrevious]][[1, 1]] === Length[sectorBoundaries],
		...
		If[ sizeNew === sectorBoundaries[[1, 2]],
			...
			subBlock =
				Table[Table[Table[rawBlock[[nVar, j, i]], {i, 1, sizeOld}], {j, sizeOld + 1, sizeNew}].trafoPrevious,
					{nVar, 1, Length[invariants]}];

			diagBlock =
				Table[Table[Table[rawBlock[[nVar, j, i]], {i, sizeOld + 1, sizeNew}], {j, sizeOld + 1, sizeNew}],
					{nVar, 1, Length[invariants]}];
			aNew =
			Table[Join[
			Function[aLine, Join[aLine, Table[0, {i, 1, sizeNew - sizeOld}]]] /@ aPrevious[[nVar]],
			MapIndexed[Function[{aLine, nLine}, Join[aLine, diagBlock[[nVar, nLine[[1]]]]]], subBlock[[nVar]]]],
				{nVar, 1, Length[invariants]}];
		];
		Return[aNew];
	];
...
CalculateNextSubsectorD[a_List, invariants_List, sectorBoundaries_List, previousD_List, userProvidedAnsatz_List: {}, OptionsPattern[]] :=
	Module[ {nextEquation, irredFactors, nLowerSector, e, c, b, subSector, alphabet, newCurrentD, nextSectorD},
		...
		irredFactors = ExtractIrreducibles[nextEquation, AllowEpsDependence -> True];
		alphabet = Union[invariants, Select[irredFactors, FreeQ[#, eps] &]];

		If[ OptionValue[VerbosityLevel] >= 8,
			Print["Transforming sector " <> ToString[nLowerSector] <> " below sector " <> ToString[subSector] <> " into dlog-form."];
		];

		If[ FreeQ[irredFactors, eps] && (And @@ Flatten[Map[PossibleZeroQ, b, {3}]] || CheckDlogForm[b, invariants, alphabet]),
			newCurrentD = (Join[Table[0, {i, 1,	sectorBoundaries[[nLowerSector, 2]] -
				sectorBoundaries[[nLowerSector, 1]] + 1}], #1] &) /@ previousD;

			If[ OptionValue[VerbosityLevel] >= 10,
				Print["Block is already in the required form."];
			];,

			nextSectorD = FindD[e, c, b, alphabet, invariants, userProvidedAnsatz,
			...
			newCurrentD = MapIndexed[Join[nextSectorD[[#2[[1]]]], #1] &, previousD];
		];
...
CalculateNonSingularPoint[alphabet_List, invariants_List] :=
	Return[FindInstance[And @@ Map[# != 0 &, alphabet], invariants][[1]]];
	...
	Module[ {startsAtOneQ, increasingQ, consecutiveQ, returnValue},
		...
		consecutiveQ =
			If[ Length[sectorBoundaries] > 1,
				And @@ Table[sectorBoundaries[[n, 2]] + 1 === sectorBoundaries[[n + 1, 1]], {n, 1, Length[sectorBoundaries] - 1}],
				True
			];
...
CheckDE[a_List, invariants_List, OptionsPattern[]] :=
	Module[ {nonNullQ, squareQ, sameDimQ, allVarsQ, varNumberMatchQ},
		...
		squareQ =
			If[ OptionValue[EnforceSquare] === True,
				(And @@ (SquareMatrixQ /@ a)),
				True
			];

		sameDimQ = (Length[Union[Dimensions /@ a]] === 1) && (Length[Dimensions[a]] === 3);
		allVarsQ = Complement[DeleteCases[Variables[a], Null], Append[invariants, eps]] === {};
		...
		Return[nonNullQ && sameDimQ && squareQ && allVarsQ && varNumberMatchQ];
	];
	...
	Module[ {dLogForm, mixedLetterPositions},
		...
		Return[(And @@
			Map[NumberQ[Simplify[#/eps]] &,
				DeleteCases[Flatten@Delete[dLogForm, mixedLetterPositions], 0]]) &&
				(DeleteCases[Flatten[Extract[dLogForm, mixedLetterPositions]], 0] === {})
		]
	];
...
CheckIntegrability[a_List, invariants_List, OptionsPattern[]] :=
	Module[ {nonSingularPoint, relation, fIntCond},
		...
		nonSingularPoint =
			...
			invariants, Integers][[1]], MapIndexed[#1 -> Prime[10^9 + #2[[1]]] &, Complement[Variables[a], invariants]]];

		relation = ((D[a[[#1[[1]]]], invariants[[#1[[2]]]]] /. nonSingularPoint) - (D[a[[#1[[2]]]],
				invariants[[#1[[1]]]]] /. nonSingularPoint) + (a[[#1[[1]]]].a[[#1[[2]]]] -
				a[[#1[[2]]]].a[[#1[[1]]]] /. nonSingularPoint) &) /@
			Flatten[DeleteCases[
			Table[Table[{i, j}, {i, j + 1, Length[invariants]}], {j, 1, Length[invariants]}], {}], 1];
...
CheckNextDsVanish[newPreviousDs_List, newPreviousSolution_List, bhat_List, lMax_Integer,
	nmin_Integer, hbar_, e_List, c_List, alphabet_List, invariants_List, delta_Integer] :=
	Module[ {ansatzD, vectorD, equationSet, vars, linearSystem, preSol, solRule},

		ansatzD = Table[Table[0, {j, 1, Length[bhat[[1, 1]]]}], {i, 1, Length[bhat[[1]]]}];
		vectorD = Join[newPreviousDs, Table[ansatzD, {i, 1, delta}]];

		equationSet =
			Flatten[Table[Table[
				Sum[(-D[SeriesCoefficient[hbar, {eps, 0, k}], invariants[[nVar]]]*vectorD[[nOrder - k - nmin + 2]] +
					SeriesCoefficient[hbar, {eps, 0, k}]*D[vectorD[[nOrder - k - nmin + 2]], invariants[[nVar]]]), {k, 0,
				Min[lMax, nOrder - nmin + 1]}] -
				Sum[SeriesCoefficient[hbar, {eps, 0, k}]*(e[[nVar]].vectorD[[nOrder - 1 - k - nmin + 2]] -
					vectorD[[nOrder - 1 - k - nmin + 2]].c[[nVar]]), {k, 0, Min[lMax, nOrder - nmin]}] +
					-Sum[g[k]*SeriesCoefficient[bhat[[nVar]], {eps, 0, nOrder - k}], {k, 0,nOrder - nmin}] -
				Sum[D[Total[Table[Log[alphabet[[nLetter]]] Table[Table[\[Alpha][nLetter, nOrder - k, i, j],
					{j, 1, Length[bhat[[1, 1]]]}], {i, 1, Length[bhat[[1]]]}], {nLetter, 1, Length[alphabet]}]],
					invariants[[nVar]]]*SeriesCoefficient[Power[hbar, 2], {eps, 0, k}], {k, 0, Min[2*lMax, nOrder - nmin]}],
				{nVar, 1, Length[invariants]}] /. newPreviousSolution, {nOrder, Length[newPreviousDs] + nmin - 1,
		...
		linearSystem =
			Map[# == 0 &, Flatten[ParallelizeMap[$ComputeParallel][RatFunctionZeroCoeffs[#, invariants] &, equationSet,
				DistributedContextsSwitch[$ComputeParallel]]]];
...
CheckNextTsVanish[aHat_List, f_, alphabet_List, invariants_List, lMIN_Integer, lMAX_Integer, (*kMAX*)_Integer,
	...
	Module[ {tVector, deAtNextDeltaOrders, equationSet, nParameterEquations, nlinearPartSolved, preTrafo, simplifiedPreTrafo,
		...
		tVector =
			Join[previousTs, Table[Table[Table[0, {j, 1, Length[aHat[[1]]]}], {i, 1, Length[aHat[[1]]]}], {i, 1, delta}]];

		deAtNextDeltaOrders =
			Table[Table[
				Sum[-D[SeriesCoefficient[f, {eps, 0, k}], invariants[[nVar]]]*tVector[[nOrder - k - lMIN + 1]] +
					SeriesCoefficient[f, {eps, 0, k}]*D[tVector[[nOrder - k - lMIN + 1]], invariants[[nVar]]], {k, lMIN, Min[lMAX, nOrder - lMIN]}] -
				Sum[SeriesCoefficient[aHat[[nVar]], {eps, 0, k}].tVector[[nOrder - k - lMIN + 1]], {k, 0, nOrder - lMIN}] +
				Sum[SeriesCoefficient[f, {eps, 0, k}]*tVector[[nOrder - k - 1 - lMIN + 1]].D[Total@Table[Log[alphabet[[nLetter]]]*
					Table[Table[\[Alpha][nLetter, i, j], {j, 1,	Length[aHat[[1]]]}], {i, 1, Length[aHat[[1]]]}], {nLetter, 1, Length[alphabet]}],
					invariants[[nVar]]] /. Dispatch[previousSolution], {k, lMIN, Min[lMAX, nOrder - 1 - lMIN]}],
				{nVar, 1, Length@invariants}], {nOrder, Length[previousTs] + lMIN, Length[previousTs] + lMIN - 1 + delta}
			];

		equationSet = Flatten[deAtNextDeltaOrders];
		nParameterEquations = (#1 == 0 &) /@ Flatten[ParallelizeMap[$ComputeParallel][RatFunctionZeroCoeffs[#1, invariants] &, equationSet,
			DistributedContextsSwitch[$ComputeParallel]]];

		nlinearPartSolved = SolveLinearPart[Join[nParameterEquations, previousNonlinears]];
		...
		preTrafo =
			Power[f, -1]*Total[MapIndexed[#1 eps^(#2[[1]] - 1 + lMIN) &,
			previousTs /. Dispatch[Join[previousSolution /. nlinearPartSolved[[1, 1]], nlinearPartSolved[[1, 1]]]]]];

		simplifiedPreTrafo = Simplify[preTrafo /. nonSingularPoint /. eps -> Prime[Power[10, 4]]];
		Off[Solve::svars];
		preSol = Solve[simplifiedPreTrafo == IdentityMatrix[Length[preTrafo]]];
		On[Solve::svars];
		...
		If[ Length[Select[Variables[preTrafo], Head[#] === \[Beta] &]] === Power[Length[preTrafo], 2],
			polishedSol =
				Nest[Function[expr, PolishStep[expr, simplifiedPreTrafo]], {{}, preSol[[1]]}, Power[Length[preTrafo], 2]][[1]];
			remSol = SolveLinearPart[nlinearPartSolved[[2]] /. Dispatch@polishedSol];

			If[ remSol =!= {} && remSol[[2]] === {},
				Return[Simplify[preTrafo /. Dispatch[remSol[[1, 1]]] /. Dispatch[polishedSol]]];,
				Return[False];
			];,

			remSol = SolveLinearPart[nlinearPartSolved[[2]] /. Dispatch[preSol[[1]]]];

			If[ ! (remSol =!= {} && remSol[[2]] === {}),
				Return[False];
			];

			freeVarsZero = Map[# -> 0 &, Variables[#[[2]] & /@ remSol[[1, 1]]]];
			fullSol = Join[remSol[[1, 1]] /. freeVarsZero, freeVarsZero];
			cmatrix = Table[Table[c[i, j], {j, 1, Length[preTrafo]}], {i, 1, Length[preTrafo]}];

			coeffList =
			DeleteCases[
				Flatten[(#1.cmatrix &) /@ (Flatten[
						Table[Table[
							Table[Table[\[Beta][nOrder, nIndep, i, j], {j, 1, Length[preTrafo]}], {i, 1, Length[preTrafo]}],
							{nIndep, 1, Length[indepFactors]}
							], {nOrder, lMIN, Length[previousTs] + lMIN - 1}], 1] /.
					Dispatch[
						Join[Join[
						Join[previousSolution /. Dispatch[nlinearPartSolved[[1, 1]]], nlinearPartSolved[[1, 1]]] /. Dispatch[preSol[[1]]],
						preSol[[1]]] /. Dispatch[fullSol], fullSol]
					])] /. {\[Beta][___] -> 0}, 0
			];

			constTrafo = cmatrix /. Nest[Function[expr, SolveConstTrafoStep[expr, cmatrix]], {{}, coeffList}, Power[Length[preTrafo], 2]][[1]];

			Return[Simplify[(preTrafo /. Dispatch[preSol[[1]]] /. Dispatch[fullSol]).constTrafo]];
		];
	];
...
ConstantNormalizationStep[{dlogForm_List, trafoPrevious_List}] :=
	Module[ {consttrafo, freeConsts, fixedPoint, trafo},
		...
		freeConsts =
			...
			Select[DeleteCases[
			Flatten[Table[Inverse[consttrafo].dlogForm[[nLetter]].consttrafo /.
				eps -> 1, {nLetter, 1, Length@dlogForm}]], 0], ! FreeQ[#, c] &];

		fixedPoint = FixedPoint[FindNextConstant, {freeConsts, {}}];
		trafo = consttrafo /. fixedPoint[[2]] /. _c -> 1;
		Return[{Table[Inverse[trafo].dlogForm[[nLetter]].trafo, {nLetter, 1, Length@dlogForm}], trafoPrevious.trafo}]
	];
...
DenominatorFactors[expr_, invariants_List, OptionsPattern[]] :=
	Module[ {factors},
		...
		If[ OptionValue[ShowPowersAndPrefactor],
			Return[{Select[factors, IndependentOfInvariantsQ[#, invariants] &],
				Select[factors, !IndependentOfInvariantsQ[#, invariants] &]}
			],
			Return[Map[#[[1]] &, Select[factors, !IndependentOfInvariantsQ[#, invariants] &]]];
		];
	];
...
EliminateLinearEqns[prevNLEquations_List, prevLinearSoln_List, vars_List] :=
	Module[ {unionEquations, linearTestRes, linearEqns, nonLinearEqns,
		...
		linearTestRes = ParallelizeMap[$ComputeParallel][LinearTest[#1[[1]]] &, unionEquations,
			DistributedContextsSwitch[$ComputeParallel]];
		...
		updatedPrevSolns = Map[#1[[1]] == #1[[2]] &, Join[prevLinearSoln /. Dispatch@linearSoln[[1]]]];
		nextLinearSoln =
			Join[Solve[updatedPrevSolns, Variables[#[[1]] & /@ updatedPrevSolns]][[1]], linearSoln[[1]]];
		On[Solve::svars];

		Return[{DeleteCases[Union[nonLinearEqns /. Dispatch[linearSoln[[1]]]], True], nextLinearSoln}];
	];
...
EmptyIntersectionQ[polyList_List, invariants_List] :=
	Module[ {returnValue},
		Off[Solve::svars];
		returnValue = Solve[And @@ Map[Function[factor, factor == 0], polyList], invariants] === {};
		On[Solve::svars];
		Return[returnValue];
	];


EpsDependentNormalizationStep[{invariants_List, trafoPrevious_List, aPrevious_List}] :=
	Module[ {preFactorsOnlyDE, offsetMatrix, factors, nonSingularEpsPoint,
		currentPowers, trafoPowers, powMatrix, ordersList, numeratorContribution,
		overallFactor, vars, sol, trafo},
		...
		preFactorsOnlyDE =
			Map[Function[expr,
				Select[FactorList[expr], ! FreeQ[#1, eps] && IndependentOfInvariantsQ[#1, invariants] &]],
			aPrevious, {3}];

		factors =
			Union[(#1[[1]] &) /@ Flatten[preFactorsOnlyDE, 3], SameTest -> (NumberQ[Simplify[#1/#2]] &)];

		nonSingularEpsPoint = CalculateNonSingularPoint[factors, {eps}];

		currentPowers =
			Map[{ExprPropLetterQ[#[[1]], invariants, factors, ShowLetter -> True][[3]], #[[2]]} &, preFactorsOnlyDE, {4}];

		offsetMatrix =
			Map[Function[expr, Exponent[Times @@ Power @@@ Select[FactorList[expr], ! FreeQ[#1, eps] &&
				!IndependentOfInvariantsQ[#1, invariants] && #[[2]] > 0 &], eps]], aPrevious, {3}];

		trafoPowers =
			Table[Table[MapIndexed[{#2[[1]], c[#2[[1]], j] - c[#2[[1]], i]} &, factors], {j, 1, Length@aPrevious[[1]]}],
				{i, 1, Length@aPrevious[[1]]}];

		powMatrix =
			ReplacePart[
			Table[MapThread[
			Map[Function[flist, {flist[[1, 1]], Total[Map[Function[x, x[[2]]], flist]]}],
				GatherBy[Join[#1, #2], First]] &, {currentPowers[[nVar]],
				trafoPowers}, 2], {nVar, 1, Length@invariants}],
			Map[# -> Table[{i, 0}, {i, 1, Length@factors}] &, Position[Map[PossibleZeroQ, aPrevious, {3}], True]]];
		...
		numeratorContribution =
			...
			Function[{powMatrixComponent, offsetComponent},
				offsetComponent +
				Total[Map[
				Function[pows, ordersList[[pows[[1]]]]*StepFunction[pows[[2]]]],
				powMatrixComponent]]], {powMatrix, offsetMatrix}, 3], 2]];

		overallFactor =
			Total[
			MapIndexed[ordersList[[#2[[1]]]]*# &,
			Map[Function[flist, -Min[Map[Function[fac, -StepFunction[-fac[[2]]]], flist]]],
			Sort[GatherBy[Flatten[powMatrix, 3], #[[1]] &], #1[[1, 1]] < #2[[1, 1]] &]]]];
	...
	(* Mathematica 12 slightly changed the syntax for NMinimze, so account for that here *)
		...
		If[ sol[[1]] - (overallFactor + numeratorContribution /. _c -> 0) > -0.2,
			trafo = IdentityMatrix[Length@aPrevious[[1]]],
			trafo = DiagonalMatrix[Table[Times @@
				MapIndexed[Power[#, c[#2[[1]], i]] &, factors], {i, 1,
				Length@aPrevious[[1]]}]] /. sol[[2]] /. _c -> 0
		];

		Return[{invariants, trafoPrevious.trafo, TransformDE[aPrevious, invariants, trafo]}];
	];


ExprPropLetterQ[expr_, invariants_List, alphabet_List, OptionsPattern[]] :=
	Module[ {remainderList, candidatePositions, selectList, propConst},
		If[ And @@ Map[PolynomialQ[#, invariants] &, Join[{expr}, alphabet]],
			remainderList =
			Map[Function[letter,
				ReleaseHold[
				And @@ Function[var,
					Hold[PossibleZeroQ[
					PolynomialRemainder[expr, letter, var]]]] /@ invariants]], alphabet];

			candidatePositions = Position[remainderList, True];

			selectList = Select[Map[{Simplify[expr/alphabet[[#[[1]]]]], #} &, candidatePositions], NumberQ[#[[1]]] &];
			If[ OptionValue[ShowLetter],
				If[ ! selectList === {},
					propConst = selectList[[1, 1]];
					Return[{True, alphabet[[selectList[[1, 2, 1]]]], selectList[[1, 2, 1]], propConst}];,
					Return[{False, {}, {}, {}}];
				];,
				If[ ! selectList === {},
					Return[True],
...
ExtractDiagonalBlock[a_List, boundaries_List] :=
	Module[ {},
		If[ ! (Length[boundaries] === 2 && And @@ Map[IntegerQ[#] && Positive[#] &,
				boundaries] && (Length[a[[1]]] >= boundaries[[2]] >= boundaries[[1]] >= 1)),
			Message[ExtractDiagonalBlock::badbounds];
			Return[False];
		];
		Return[Map[Function[delA, Take[Map[Take[#, boundaries] &, delA], boundaries]], a]];
	];
...
FindAndApplyRelations[alphabet_List, (*invariants*)_List, expr_, preferredTerms_List] :=
	Module[ {occuringPreferredTerms, nonOccuringPreferredTerms, extExpr,
		...
		If[ Length@expr === 1,
			Return[expr];,
			occuringPreferredTerms = Map[#[[2]] &, Select[expr, MemberQ[preferredTerms, #[[2]]] &]];
			nonOccuringPreferredTerms = Complement[preferredTerms, occuringPreferredTerms];
			extExpr = Join[expr, Map[{0, #} &, nonOccuringPreferredTerms]];
			terms = Map[#[[2]] &, extExpr];
			positionsPreferredterms = Position[terms, #][[1, 1]] & /@ preferredTerms;
			coefficients = Map[#[[1]] &, extExpr];
			toLCDFactor = Times @@ (Select[(Sort[#1, #1[[2]] > #2[[2]] &][[-1]] &) /@
				GatherBy[Flatten[terms, 1], #1[[1]] &], #1[[2]] < 0 &] /.
				Fac[nLetter_, pow_] :> Power[alphabet[[nLetter]], -pow]);

			termsLCD =
				Map[toLCDFactor* Times @@ (#1 /. Fac[nLetter_, pow_] :>
					Power[alphabet[[nLetter]], pow]) &, terms];
			...
			solution =
			Map[#[[1]] -> #[[2]] &,
			Apply[List, Reduce[Function[liste,
			...
			If[ solution === True,
				...
				eliminatedSol = solution /. Map[# -> -coefficients[[#[[1]]]] &, indepVariables];
				finalExpr =
					Select[Map[{#[[2]] + coefficients[[#[[1, 1]]]], terms[[#[[1, 1]]]]} &, eliminatedSol], #[[1]] =!= 0 &];
				Return[finalExpr];
			];
		];
	];
...
FindAnsatzSubsectorD[a_List, invariants_List, sectorBoundaries_List, previousD_List, OptionsPattern[]] :=
...
FindAnsatzT[a_List, invariants_List, OptionsPattern[]] :=
	Module[ {irreducibleDenominators, alphabet, mixedLetters, trDlog,
		...
		StartKernels[];
		If[ ! (IntegerQ[OptionValue[TDeltaDenominatorDegree]] && OptionValue[TDeltaDenominatorDegree] >= 0),
			Message[FindAnsatzT::nintden];
			Return[False];
		];
		If[ ! (IntegerQ[OptionValue[TDeltaNumeratorDegree]] && OptionValue[TDeltaNumeratorDegree] >= 0),
			Message[FindAnsatzT::nintnum];
			Return[False];
		];

		irreducibleDenominators = ExtractIrreducibles[a, AllowEpsDependence -> True];
		alphabet = Select[irreducibleDenominators, FreeQ[#, eps] &];
		mixedLetters = Select[irreducibleDenominators, ! FreeQ[#, eps] &];
		trDlog = CalculateDlogForm[({{Tr[#]}} &) /@ a, invariants, Join[alphabet, mixedLetters]];
		...
		trDlogZero = SeriesCoefficient[Take[trDlog, Length[alphabet]], {eps, 0, 0}];

		maxNegativePowers =
			Select[MapIndexed[{#2[[1]],
				If[ #[[1, 1]] >= 0,
					-OptionValue[TDeltaDenominatorDegree],
					#[[1, 1]] -	OptionValue[TDeltaDenominatorDegree]
				]} &,
			trDlogZero], #[[2]] < 0 &];
		...
		posPowerFactorsv2 =
			Map[#[[2]] &,
			Select[Function[monomial, {Total[(#[[2]] &) /@
				Select[FactorList[monomial], ! NumberQ[#[[1]]] &]], monomial}] /@
		...
		posPowerFactors = Union[posPowerFactorsv1, posPowerFactorsv2, posPowerFactorsv3];
...
FindConstantNormalization[invariants_List, trafoPrevious_List, aPrevious_List, OptionsPattern[]] :=
	Module[ {alphabet, dlogForm, trafo},
		...
		trafo = FixedPoint[ConstantNormalizationStep, {dlogForm, IdentityMatrix[Length@dlogForm[[1]]]}][[2]];

		Return[{trafoPrevious.trafo, TransformDE[aPrevious, invariants, trafo]}];
	];


FindD[e_List, c_List, b_List, alphabet_List, invariants_List, userProvidedAnsatz_: {}, OptionsPattern[]] :=
	Module[ {bhatAndFactors, bhat, hbar, khat, nmin, res, dhat, fullD, indepFactors},
		...
		res = NestWhile[
			...
			indepFactors, {previousResult[[1]], previousResult[[2]], previousResult[[3]]},
		...
		dhat = Sum[res[[1, n - nmin + 2]]*Power[eps, n], {n, nmin - 1, Length[res[[1]]] + nmin - 2}];
...
FindEpsDependentNormalization[a_List, invariants_List] :=
	Module[ {fixedPoint},
		fixedPoint =
			FixedPoint[EpsDependentNormalizationStep, {invariants, IdentityMatrix[Length[a[[1]]]], a}];
...
FindNextConstant[{freeConsts_List, previousSol_List}] :=
	Module[ {minimizingFactors},
		...
		If[ Length[minimizingFactors] > 0 && minimizingFactors[[1, 2, 2]] > 0,
			Return[{freeConsts /. minimizingFactors[[1, 1]] -> minimizingFactors[[1, 2, 1]],
				Append[previousSol, minimizingFactors[[1, 1]] -> minimizingFactors[[1, 2, 1]]]}];,
...
ForwardSolve[{equations_, prevRules_List, vars_List}] :=
	Module[ {allVars, allWithFirstVar, solveVars, sol, rules, newSys},
		...
		solveVars = Intersection[vars, Variables[Map[#[[1]] &, allWithFirstVar]]];
...
GatherByDenominator[indepFactorForm_List] :=
	Return[GatherBy[indepFactorForm, Function[summand, Select[summand[[2]], #[[2]] < 0 &]]]];


GatherCommonTerms[indepFactorForm_List] :=
	Module[ {sortedTerms},
		sortedTerms =
			Map[Function[term, {term[[1]], Sort[term[[2]], #1[[1]] < #2[[1]] &]}], indepFactorForm];
		Return[
			Select[Map[
			Function[
			gatheredTerms, {Total@
...
GatherCommonTermsRaw[termList_List, invariants_List] :=
	Module[ {factorsList, splittedFactorsList},
		...
		splittedFactorsList =
			Function[factors, {Union[Select[factors, IndependentOfInvariantsQ[#, invariants] &]],
			Union[Select[factors, ! IndependentOfInvariantsQ[#, invariants] &]]}] /@ factorsList;
...
GenericPolynomial[invariants_List, highestDegree_Integer, identifier_] :=
	Module[ {partitions},
		partitions =
			Map[Function[degree,
			Flatten[Map[
...
InsertBlockIntoIdentity[blockMatrix_List, dim_Integer, blockStartPosition_Integer] :=
...
InsertDIntoIdentity[sectorBoundaries_List, fullD_List] :=
	Module[ {trafoSize, trafo},
		trafoSize =
			sectorBoundaries[[Position[sectorBoundaries, Length[fullD[[1]]]][[1, 1]] + 1]][[2]];
		trafo =
...
LinearSystemSolver[parameterEquations_List, vars_List, maxLength_Integer] :=
	Module[ {preforwardSolved, sol, forwardSolved},
		...
		sol = Solve[preforwardSolved[[1]], Variables[Map[#[[1]] &, preforwardSolved[[1]]]]];
		...
		Return[{Fold[Join[#2 /. Dispatch[#1], #1] &, {}, Reverse[forwardSolved]]}];
	];
...
MinimizePrimeFactors[ratList_List] :=
	Module[ {facIntList, nPowersTotal, primeFactors, minSolution,
		...
		minimizingFactor = Times @@ (Power[#[[1]], #[[2, 2, 1, 2]]] & /@ minSolution);
...
NextBlockBoundaries[aPrevious_List, sectorBoundaries_List] :=
	If[ Length[aPrevious[[1]]] === 0,
		...
		Return[sectorBoundaries[[Position[sectorBoundaries, Length[aPrevious[[1]]]][[1, 1]] + 1]]]
	];


NextEquationD[a_List, invariants_List, sectorBoundaries_List, previousD_List] :=
	Module[ {position, subSector, subSectorSize, nLowerSector, e,
		c, b, newc, cmix, bbar},
		...
		subSectorSize = sectorBoundaries[[subSector, 2]] - sectorBoundaries[[subSector, 1]] + 1;
		...
		nLowerSector =
			Position[sectorBoundaries, sectorBoundaries[[subSector, 1]] - Length[previousD[[1]]] - 1][[1, 1]];

		e = SeriesCoefficient[ExtractDiagonalBlock[a, {Length[a[[1]]] - subSectorSize + 1, Length[a[[1]]]}], {eps, 0, 1}];
		c = SeriesCoefficient[ExtractDiagonalBlock[a, {1, Length[a[[1]]] - subSectorSize}], {eps, 0, 1}];
		b = Function[M,
			Take[(Take[#1, {sectorBoundaries[[nLowerSector, 1]],
					sectorBoundaries[[nLowerSector, 2]]}] &) /@
				M, {Length[M] - subSectorSize + 1, Length[M]}]] /@ a;

		newc = ExtractDiagonalBlock[c, {sectorBoundaries[[nLowerSector]][[1]],sectorBoundaries[[nLowerSector]][[2]]}];
		...
		If[ sectorBoundaries[[nLowerSector]][[2]] == Length[c[[1]]],
			...
			bbar = b + Table[-eps previousD.cmix[[nVar]], {nVar, 1, Length[invariants]}];
		];
		...
		For[highestDegree = 0, solution == {}, highestDegree++,
			coeffList =
				Flatten[CoefficientList[
					Total[MapIndexed[GenericPolynomial[invariants, highestDegree, #2[[1]]]*# &, polyList]],
					invariants], 2];
			coefficients =
			Flatten[Table[
			Table[Table[
				...
				Length[Flatten[
					...
					degree]]}], {degree, 0, highestDegree}], {nPoly, 1, Length@polyList}], 2];
			solution =
				Solve[coeffList[[1]] == 1 && And @@ Map[# == 0 &, Drop[coeffList, 1]], coefficients];
		];
		On[Solve::svars];
		additionalRules =
			Map[# -> 0 &, Select[Variables[Map[#[[2]] &, solution[[1]]]], Head[#] === c &]];
		minimalSolution =
			Join[solution[[1]] /. additionalRules, additionalRules];
		certificate =
			MapIndexed[GenericPolynomial[invariants, highestDegree - 1, #2[[1]]] &, polyList] /. minimalSolution;
		Return[certificate];
	];
...
RatFunctionZeroCoeffs[rawEquation_, invariants_List] :=
	Module[ {totalDenNumList, preDenNumList, epsDepDens, denNumList,
		...
		totalDenNumList = ({Numerator[#], Denominator[#]} &) /@ SummandList[Expand[rawEquation]];
		preDenNumList =
			Function[sameDenGroup, {Total[Function[term, term[[1]]] /@ sameDenGroup],
		...
		occuringFactors =
			Union[Select[Union[Flatten[FactorList[#[[2]]] & /@ denNumList]], !NumberQ[#] &],
				SameTest -> (NumberQ@Simplify[#1/#2] &)];

		factorFormList = ({#[[1]], ToFactorForm[#[[2]], occuringFactors, invariants]} &) /@ denNumList;

		cleanFactorList =
			Function[sameDenGroup, {Total[Function[term, term[[1]]] /@ sameDenGroup], sameDenGroup[[1, 2]]}] /@
			GatherBy[({#[[1]]/#[[2, 1]], #[[2, 2]]} &) /@ factorFormList, #[[2]] &];
		...
		expandedPoly = Total[Expand /@ MapIndexed[#[[1]]*transitionFactors[[#2[[1]]]] &,
			cleanFactorList]];

		Return[Union[(#[[2]] &) /@ CoefficientRules[expandedPoly, invariants]]];
	];


RecursivelyTransformSectors[aFull_List, invariants_List, sectorBoundaries_List, {nSecStart_Integer?Positive,
	nSecStop_Integer?Positive}, trafoPrevious_List: {}, aPrevious_List: {{}}, OptionsPattern[]] :=
	Module[ {previousSize, resultFull, resultFullNormalized},
		...
		If[ OptionValue[FinalConstantNormalization] === True,
			resultFullNormalized = FindConstantNormalization[invariants, resultFull[[1]], resultFull[[2]]];
...
ShrinkEquationSet[parameterEquations_List, vars_List] :=
	Module[ {expandedExtractedParameterEqns, singleTerms, termstoC,
		...
		newEqns = coeffMatrixReduced.Prepend[Table[xc[i], {i, 1, Length[termstoC]}], 1];
		paramEqns = (#1 == 0 &) /@ newEqns /. Dispatch[ctoTerms];
		Return[DeleteCases[paramEqns, True]]
	];


ShrinkNElimLinearStep[{prevNLEquations_List, prevLinearSoln_List, vars_List}] :=
...
SolveConstTrafoStep[{prevSol_List, currentCoeff_}, cmatrix_List] :=
	Module[ {val, det, sol},
		...
		det = 0;
		While[det === 0,
			sol = Solve[currentCoeff[[1]] == val][[1]];
			det = Det[cmatrix /. prevSol /. sol /.
		...
		Return[{Join[prevSol /. sol, sol], Select[Simplify[currentCoeff /. sol], ! FreeQ[#, c] &]}]
	];
...
SolveSimpleEqns[{unsolvedLinearEqns_, solvedLinearEqns_, vars_}] :=
	Module[ {newSimpleEqns, solNewSimpleEqns, rules, nSys},
		...
		Return[{nSys, Append[solvedLinearEqns, solNewSimpleEqns[[1]]], vars}];
	];
...
SplitAlgebraicallyDependent[expr_, invariants_List] :=
	Module[ {denFactors, nonConstantDenFactors, constantDenFactor,
		...
		denFactors = DenominatorFactors[expr, invariants, ShowPowersAndPrefactor -> True];
		nonConstantDenFactors = (Power[#[[1]], #[[2]]] &) /@ denFactors[[2]];
		constantDenFactor = Times @@ (Power[#[[1]], #[[2]]] &) /@ denFactors[[1]];

		denLetters = (#1[[1]] &) /@ denFactors[[2]];
		algIndepTest = AlgebraicallyIndepQ[denLetters, invariants, ShowAnnihilatingPoly -> False];
...
SplitEpsFactors[expr_List, invariants_List] :=
	Module[ {fullFactorList, indepFactorList, factor},
		...
		indepFactorList = Select[fullFactorList, IndependentOfInvariantsQ[#, invariants] &];
...
SplitNumerator[group_List, alphabet_List, invariants_List] :=
	Module[ {num, backSubstitutionRules, denFactors, allReductions,
		...
		num = Total@
			...
			group] /. Fac[nLetter_, pow_] :> Power[alphabet[[nLetter]], pow];

		If[ IndependentOfInvariantsQ[num, invariants],
			Return[group];,
			backSubstitutionRules =
				MapIndexed[Function[{term, nTerm}, prefactor[nTerm[[1]]] -> term[[1]]], group];
			...
			constantRemainderReductions =
				Select[allReductions, FreeQ[#[[2]], var_ /; MemberQ[invariants, var]] &];

			reduction =
				Join[remainderLessReductions, constantRemainderReductions, allReductions][[1]];
			...
			remainder =
				Map[{#[[1]], Join[#[[2]], denFactors]} &, ToIndepFactorForm[reduction[[2]], alphabet, invariants]];
			Return[Select[Join[containedInIdeal, remainder], ! #[[1]] === 0 &]];
		];
	];
...
ToFactorForm[expr_, alphabet_List, invariants_List] :=
	Module[ {separated, factors, constantFactors, nonConstantFactors,
		...
		separated = SeparateNumericalFactors[expr, {\[Alpha], \[Beta], eps}];
		...
		prefactor = separated[[1]]*Times @@ Apply[Power, constantFactors, {1}] Times @@
...
ToIndepFactorForm[expr_, alphabet_List, invariants_List] :=
	Return[Map[ToFactorForm[#, alphabet, invariants] &, RepeatedlySplitExpr[expr, invariants]]];
...
userProvidedAnsatz_List: {}, OptionsPattern[]] :=
	Module[ {irreducibleDenominators, alphabet, normRes, aNormed,
		...
		If[ a === Table[Table[Table[0, {j, 1, Length[a[[1]]]}],
				{i, 1, Length[a[[1]]]}], {k, 1, Length[invariants]}],
		...
		aNormed = normRes[[2]];
		trDlog =
			CalculateDlogForm[{{Tr[#]}} & /@ aNormed, invariants, Join[alphabet, mixedLetters]];
		If[ trDlog === False,
			Message[TransformDiagonalBlock::noepsform];
			Return[False];
		];
		nonZeroTrCoeffs = DeleteCases[Map[#[[1, 1]] &, Take[trDlog, Length[alphabet]]], 0];
		...
		mixedContribution =
			Times @@ MapIndexed[mixedLetters[[#2[[1]]]]^#1 &, Flatten[Drop[trDlog, Length[alphabet]]]];

		pureContribution =
			Times @@ MapIndexed[alphabet[[#2[[1]]]]^#1 &,
			Flatten[SeriesCoefficient[Take[trDlog, Length[alphabet]], {eps, 0, 0}]]];
		...
		kMAX = Sort[Select[Flatten[Exponent[aHat, eps, Max]], NumberQ[#1] &]][[-1]];
		kMIN = Sort[Select[Flatten[Exponent[aHat, eps, Min]], NumberQ[#1] &]][[1]];
...
On[Solve::svars];
		...
		Return[{Simplify[normRes[[1]].trafo], TransformDE[aNormed, invariants, trafo]}];
	];


TransformDlogToEpsForm[invariants_List, sectorBoundaries_List, trafoPrevious_List,
	...
	Module[ {alphabet, pretAnsatz, subSector, enforceBlockTriangularRule,
		...
		If[ subSector =!= 1,
			If[ OptionValue[VerbosityLevel] >= 8,
				Print["Transforming sector " <> ToString[subSector] <> " from dlog-form into epsilon-form."]
			]
		];
		alphabet = ExtractIrreducibles[aPrevious];
		dLogForm = CalculateDlogForm[aPrevious, invariants, alphabet];
		...
		If[ And @@ (NumberQ[Simplify[#1/eps]] &) /@ DeleteCases[Flatten[dLogForm], 0],
			Return[{trafoPrevious, aPrevious}];
		];

		pretAnsatz =
			Table[t[i, j], {i, 1, Length[dLogForm[[1]]]}, {j, 1, Length[dLogForm[[1]]]}];
		...
		generalSolution =
			Simplify[
			tAnsatz /.
			Solve[And @@ Map[#.tAnsatz/eps - (tAnsatz.#/mu /. eps -> mu) == 0 &,
				dLogForm],
				Variables[Table[t[i, j], {i, 1, Length[dLogForm[[1]]]}, {j, 1,
				Length[dLogForm[[1]]]}]]][[1]]];
		On[Solve::svars];
...
TransformIntoBlockTriangularForm[a_List, invariants_List] :=
	Module[ {adjMatrix, adjGraph, components, condensateEdges,
		...
		sectorBoundaries =
			Drop[FoldList[
			Function[{prev, delta}, {prev[[2]] + 1, prev[[2]] + delta}], {0, 0}, Length /@ newPartition], 1];

		Return[{sectorBoundaries, {trafo, TransformDE[a, invariants, trafo]}}];
	];


TransformNextDiagonalBlock[aFull_List, invariants_List, sectorBoundaries_List, trafoPrevious_List, aPrevious_List,
	userProvidedAnsatz_List: {}, OptionsPattern[]] :=
	Module[ {aNew, nextBlockBounds, newBlock, subSector,
		...
		aNew = CalculateNexta[aFull, invariants, sectorBoundaries, trafoPrevious, aPrevious];
		If[ aNew === False,
			Return[{False, {trafoPrevious, aPrevious}}];
		];
		nextBlockBounds = NextBlockBoundaries[aPrevious, sectorBoundaries];
		...
		If[ OptionValue[VerbosityLevel] >= 6,
			Print["Transforming diagonal block of sector " <> ToString[subSector] <> "."]
		];

		trafoLowerSectorsToEpsForm = InsertBlockIntoIdentity[trafoPrevious, Length[aNew[[1]]], 1];
		newBlockInEpsForm =
		...
		trafoBlockToEpsForm =
			InsertBlockIntoIdentity[newBlockInEpsForm[[1]], Length[aNew[[1]]], nextBlockBounds[[1]]];

		aBlockInEpsForm = TransformDE[aNew, invariants, trafoBlockToEpsForm];
		Return[{trafoLowerSectorsToEpsForm.trafoBlockToEpsForm, aBlockInEpsForm}];
	];


TransformNextSector[aFull_List, invariants_List, sectorBoundaries_List, trafoPrevious_, aPrevious_,
	userProvidedAnsatz_List: {}, OptionsPattern[]] :=
	Module[ {aBlockInEpsForm, aBlockInEpsFormOffDiagIndLog, aInEpsForm,
		...
		Return[{Simplify[aInSimplifiedEpsForm[[1]]], aInSimplifiedEpsForm[[2]]}];
	];


TransformOffDiagonalBlock[invariants_List, sectorBoundaries_List, trafoPrevious_List,
	aPrevious_List, userProvidedD_List: {}, OptionsPattern[]] :=
	Module[ {position, subSector, irredFactors, subSectorSize,
		...
		subSectorSize =
			sectorBoundaries[[subSector, 2]] - sectorBoundaries[[subSector, 1]] + 1;
		...
		irredFactors =
			ExtractIrreducibles[offdiagonalBlock, AllowEpsDependence -> True];
		...
		Return[{trafoPrevious.trafoD, TransformDE[aPrevious, invariants, trafoD]}];
	];
